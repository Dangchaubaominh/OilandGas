...
const visual3DApi = {
  ...
  upload3DModel(id, fileData) {
    const url = `/3d/instruments/${id}/upload`;
  ...
  getModelProcessingStatus(id) {
    const url = `/3d/instruments/${id}/status`;
    return axiosClient.get(url);
  },

  // Lấy cấu hình load mô hình cho Frontend
  getModelConfig(id) {
    const url = `/3d/instruments/${id}/config`;
    return axiosClient.get(url);
  },
};
